...
public class Scholar_Consolation : ISlotResolver
{
    ...
    public int Check()
    {
        ...
        if (SpellsDefine.Consolation.RecentlyUsed(3000)) return -1;
        if (Core.Get<IMemApiScholar>().SeraphTimer() < 22000 && Core.Get<IMemApiSpell>().GetCharges(SpellsDefine.Consolation.GetSpell().Id) == 2 && Core.Get<IMemApiScholar>().SeraphTimer() > 0) return 1;
