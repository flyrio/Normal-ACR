...
public class Machinist_GaussRound : ISlotResolver
{
    ...
    public int Check()
    {
        if (Core.Get<IMemApiSpell>().GetActionInRangeOrLoS(SpellsDefine.GaussRound) == 566) return -1;
        if (!Qt.GetQt("攒资源") && Core.Get<IMemApiSpell>().GetCharges(2874) >= 1 && Core.Me.ClassLevel >= 15 && Core.Me.ClassLevel < 74 && AI.Instance.GetGCDCooldown() >= 600) return 1;
