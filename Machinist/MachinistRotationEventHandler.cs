...
public class MachinistRotationEventHandler : IRotationEventHandler
{
    ...
    public async Task OnNoTarget()
    {
        ...
        if (!Qt.GetQt("主动攻击"))
            Share.Pull = false;
        var slot = new Slot();
        if (SpellsDefine.Peloton.IsReady() && !SpellsDefine.Peloton.RecentlyUsed() && !Core.Me.HasAura(1199) && Core.Get<IMemApiMove>().IsMoving() && !Core.Get<IMemApiCondition>().IsInCombat() && Core.Me.HitboxRadius() == 0.5 && Qt.GetQt("自动速行"))
        {
